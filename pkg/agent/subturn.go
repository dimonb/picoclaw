...
func (e *ephemeralSessionStore) GetContextSnapshot(_ string) session.ContextSnapshot {
	e.mu.Lock()
	defer e.mu.Unlock()
	history := make([]providers.Message, len(e.history))
...
func (e *ephemeralSessionStore) ApplySummaryCompaction(
	_ string,
	summary string,
	expectedHistoryCount, keepLast int,
) session.SummaryCompactionResult {
	e.mu.Lock()
	defer e.mu.Unlock()

	result := session.SummaryCompactionResult{
		Applied:            len(e.history) == expectedHistoryCount,
		HistoryCountBefore: len(e.history),
	}
	if !result.Applied {
		result.HistoryCountAfter = len(e.history)
		return result
	}

	e.summary = summary
	if keepLast <= 0 {
		result.SummarizedCount = len(e.history)
		e.history = nil
	} else if keepLast < len(e.history) {
		result.SummarizedCount = len(e.history) - keepLast
		e.history = append([]providers.Message(nil), e.history[len(e.history)-keepLast:]...)
	}
	result.HistoryCountAfter = len(e.history)
	return result
}
