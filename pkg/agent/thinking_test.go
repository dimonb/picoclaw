...
func TestProcessMessage_ThinkCommandSetsSessionLevel(t *testing.T) {
	...
	if !strings.Contains(resp.Content, "high") {
		t.Fatalf("response = %q, want contains high", resp.Content)
	}
...
func TestProcessMessage_ThinkXHighRejectedForNonWhitelistModel(t *testing.T) {
	...
	if !strings.Contains(strings.ToLower(resp.Content), "xhigh") {
		t.Fatalf("response = %q, want xhigh hint", resp.Content)
	}
}
