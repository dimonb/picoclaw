	...
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"message_id": map[string]any{
				"type":        "string",
				"description": "Optional explicit Telegram message ID override. If omitted, the tool reacts to the current inbound message from context.",
			},
...
func (t *ReactionTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	messageID, err := resolveReactionMessageID(ctx, args)
	if err != nil {
...
func resolveReactionMessageID(ctx context.Context, args map[string]any) (string, error) {
	if messageID, _ := args["message_id"].(string); strings.TrimSpace(messageID) != "" {
		return strings.TrimSpace(messageID), nil
	}
	if id := strings.TrimSpace(ToolCurrentMessageID(ctx)); id != "" {
		return id, nil
	}
	return "", fmt.Errorf("current message id is unavailable")
}
