...
// The tool no longer bounds text itself: bounding is the registry's job, and
// a cut here would lose the tail before the spill file is written.
func TestMCPTool_Execute_LargeTextPassedThroughWhole(t *testing.T) {
	...
	if result.ForLLM != strings.TrimSpace(largeText) {
		t.Fatalf("expected the whole text in ForLLM, got %d chars", len(result.ForLLM))
	}
	if len(result.ArtifactTags) != 0 {
		t.Fatalf("expected no artifact tags, got %+v", result.ArtifactTags)
	}
}
