...
type OutboundMessage struct {
	...
	// msgIDs are the platform message IDs returned by the channel in delivery order
	// (or nil when the channel does not expose them). Used to write the assistant
	// message to the session journal only after confirmed delivery, so the
	// journal stays consistent with what was received.
