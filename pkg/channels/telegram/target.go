...
func parseTelegramTarget(raw string) (telegramTarget, error) {
	...
	legacySlash := false
	if !hasTopic {
		base, topic, hasTopic = strings.Cut(raw, "/")
		legacySlash = hasTopic
	}
	chatID, err := strconv.ParseInt(strings.TrimSpace(base), 10, 64)
	if err != nil {
	...
	if err != nil {
		if legacySlash {
			return telegramTarget{}, fmt.Errorf("invalid thread ID in chat ID %q: %w", raw, err)
		}
		return telegramTarget{}, fmt.Errorf("invalid telegram topic ID %q: %w", topic, err)
	}
	if threadID <= 0 {
		if legacySlash {
			return telegramTarget{}, fmt.Errorf("invalid thread ID %d", threadID)
		}
