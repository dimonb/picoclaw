...
func newAddCommand(storePath func() string) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			job, err := cs.AddJob(name, schedule, message, mode, channel, to, "")
			if err != nil {
	...
	cmd.Flags().StringVarP(&mode, "mode", "d", cron.ModeAgent, "Execution mode: 'agent' re-enters the agent, 'direct' posts the message as-is")
