...
能够自行分页返回结果的工具不会写文件：`read_file` 读取的文件本来就在磁盘上，并支持通过 `offset`/`length`（字节模式）或 `start_line`/`max_lines`（行模式）分段读取，因此超限结果同样会被裁剪为首尾预览，但不会写副本，提示信息会引导模型再次读取同一路径。`web_fetch` 不在豁免之列：它的内容不存在于其他地方，文件是唯一的取回方式。工具通过实现 `SelfPagingTool` 并返回说明自身分页参数的提示语来声明该豁免。
